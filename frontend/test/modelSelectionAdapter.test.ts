...
describe("modelSelectionAdapter", () => {
  describe("enterMulti", () => {
    ...
    it("should preserve existing arrays if not empty", () => {
      ...
      expect(result.judge_models).toEqual(["grok-4.3"]); // Seeded from last_single_judge
    });
  ...
  describe("leaveMulti", () => {
    ...
    it("should capture first selected from each array to last_single_*", () => {
      const settings: ModelSettings = {
        ...
        multi_model_enabled: true,
        fit_models: ["gpt-5", "claude-4.1-opus"],
        tailor_models: ["gemini-3.1-pro-preview", "grok-4.3"],
        judge_models: ["gpt-5"],
      };
  ...
  describe("selectAll", () => {
    it("should replace role array with all model IDs", () => {
      ...
      const allModels = ["gpt-5", "claude-4.1-opus", "gemini-3.1-pro-preview", "grok-4.3"];
