...
def test_gpt52_chat_latest_stored_correctly(db_session: Session):
    """Verify that gpt-5.2-chat-latest virtual model ID is stored in charges table."""
    user = _mk_user(db_session)
    job = _mk_job(db_session, user.id)
    pm = load_price_map()
    
    # Simulate recording a charge for GPT-5.2 Chat Latest using gpt-5 pricing (exists in config)
    record_charge_for_job(
        ...
        request_type="fit",
        provider="openai",
        model="gpt-5.2-chat-latest",  # Virtual model ID from frontend
        prompt_tokens=1000,
    ...
    assert charge.model == "gpt-5.2-chat-latest", f"Expected 'gpt-5.2-chat-latest' but got '{charge.model}' - virtual ID not preserved!"
    assert charge.provider == "openai"


def test_gpt52_stored_correctly(db_session: Session):
    """Verify that gpt-5.2 virtual model ID is stored in charges table."""
    user = _mk_user(db_session)
    job = _mk_job(db_session, user.id)
    pm = load_price_map()
    ...
    record_charge_for_job(
        ...
        request_type="fit",
        provider="openai",
        model="gpt-5.2",  # Virtual model ID from frontend
        prompt_tokens=1000,
        completion_tokens=500,
        price_map=pm,
        pricing_version=int(pm.get("version", 1)),
    ...
    assert charge is not None
    assert charge.model == "gpt-5.2", f"Expected 'gpt-5.2' but got '{charge.model}'"
    assert charge.provider == "openai"


def test_gpt52_models_distinct_in_analytics(db_session: Session):
    """Verify that gpt-5.2-chat-latest and gpt-5.2 appear as separate entries in analytics."""
    user = _mk_user(db_session)
    ...
    distinct_models = db_session.query(Charge.model).filter(
        ...
        Charge.model.like("gpt-5.2%")
    ).distinct().all()
    
    model_names = [m[0] for m in distinct_models]
    
    assert "gpt-5.2-chat-latest" in model_names, "gpt-5.2-chat-latest should appear in analytics"
    assert "gpt-5.2" in model_names, "gpt-5.2 should appear in analytics"
